    ...
    <?php if ( have_posts() ) : while ( have_posts() ) : the_post(); ?>
        <article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
            <header class="header">
                ...
                <a href="<?php echo esc_url( get_permalink( $post->post_parent ) ); ?>" title="<?php _e('Return to' . esc_html( get_the_title( $post->post_parent ), 1 ) , 'ucla-dcp') ?>" rev="attachment"><span class="meta-nav">&larr;</span><?php _e('Return to ' . get_the_title( $post->post_parent ), 'ucla-dcp' ) ?></a>
